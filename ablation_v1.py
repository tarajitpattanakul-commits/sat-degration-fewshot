...
TRACK_A_SEEDS = [42, 123, 7, 2024, 99]  # 5 seeds
...
TRACK_B_TEST_DEGRADATION = ["none", "severe"]   # evaluate on BOTH per trained model
TRACK_B_SEEDS = [42, 123, 7, 2024, 99]  # 5 seeds
...
def open_resumable_csv(path, fieldnames):
    ...
    (file_handle, writer, set_of_already_done_keys) for resume support."""
...
def run_track_a():
    ...
    LR (1e-4 instead of 1e-3), to check whether the original LR=1e-3 results
    were a fine-tuning-quality confound rather than a genuine architecture
    difference."""
    print("=" * 60)
    print("TRACK A: LR-tuned rerun (mobilenet, vit @ lr=1e-4)")
    print("=" * 60)

    fieldnames = ["model", "shots", "degradation", "seed", "lr",
                  "accuracy", "f1", "train_time_sec", "num_params"]
    f, writer, already_done = open_resumable_csv(TRACK_A_CSV, fieldnames)

    for model_name in TRACK_A_MODELS:
        ...
        for n_shot in TRACK_A_SHOTS:
            for degradation in TRACK_A_DEGRADATION:
                for seed in TRACK_A_SEEDS:
                    shots_label = n_shot if n_shot else "full"
                    key = (model_name, str(shots_label), degradation, str(seed), str(lr))
                    if key in already_done:
                        print(f"Skipping (already done): {key}")
                        continue

                    print(f"[Track A] model={model_name}, shots={shots_label}, "
    ...
    f.close()
    print(f"\nTrack A complete. Results in {TRACK_A_CSV}")


def run_track_b():
    """Trains models under one degradation condition, then evaluates each
    trained model on BOTH test-degradation conditions (not just the matching
    one). This separates train-time vs. test-time contributions to the floor
    effect - the original study only ever tested the matched diagonal."""
    print("=" * 60)
    print("TRACK B: Train/test degradation decoupling ablation")
    print("=" * 60)

    fieldnames = ["model", "shots", "train_degradation", "test_degradation", "seed", "lr",
                  "accuracy", "f1", "train_time_sec", "num_params"]
    f, writer, already_done = open_resumable_csv(TRACK_B_CSV, fieldnames)

    for model_name in TRACK_B_MODELS:
        cfg = MODEL_CONFIG[model_name]
        base_dataset = load_eurosat(image_size=cfg["image_size"], normalize=False)
        train_size = int(0.8 * len(base_dataset))
        test_size = len(base_dataset) - train_size
        ...
        for n_shot in TRACK_B_SHOTS:
            for train_deg in TRACK_B_TRAIN_DEGRADATION:
                for seed in TRACK_B_SEEDS:
                    ...
                    if not needed_test_degs:
                        print(f"Skipping (already done): {model_name}, {shots_label}, train={train_deg}, seed={seed}")
                        continue

                    print(f"[Track B] model={model_name}, shots={shots_label}, "
                          f"train_degradation={train_deg}, seed={seed}, lr={lr}")
                    set_all_seeds(seed)

                    train_degraded = make_degraded_dataset(train_base, train_deg, normalize_after)
                    train_subset = make_few_shot_subset(train_degraded, n_shot, seed)
                    loader_kwargs = dict(num_workers=2, pin_memory=(DEVICE == "cuda"), persistent_workers=True)
                    train_loader = DataLoader(train_subset, batch_size=32, shuffle=True, **loader_kwargs)
                    ...
                    for test_deg in needed_test_degs:
                        ...
                        test_loader = DataLoader(test_degraded, batch_size=64, **loader_kwargs)
                        accuracy, f1 = evaluate(trained_model, test_loader)

                        row = {
                            "model": model_name, "shots": shots_label,
                            "train_degradation": train_deg, "test_degradation": test_deg,
                            "seed": seed, "lr": lr,
                            "accuracy": accuracy, "f1": f1,
                            "train_time_sec": round(train_time, 1), "num_params": num_params,
                        }
                        writer.writerow(row)
                        f.flush()
                        print(f"  -> test_degradation={test_deg}: acc={accuracy:.3f} f1={f1:.3f}")

    f.close()
    print(f"\nTrack B complete. Results in {TRACK_B_CSV}")
