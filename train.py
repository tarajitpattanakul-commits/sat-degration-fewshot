...
def train_and_eval(model, train_loader, test_loader, epochs, lr=1e-3):
    ...
    start = time.time()
    # Print progress every few epochs (or every epoch for short runs) so a
    # long-running cell isn't fully silent - helps distinguish "still working"
    # from "actually stuck" during long full-data / heavy-model runs.
...
SEEDS = [42, 123, 7, 2024, 99, 17, 256, 512, 88, 314]  # 10 seeds

RAW_CSV = "/content/drive/MyDrive/results_raw.csv"
SUMMARY_CSV = "/content/drive/MyDrive/results_summary.csv"
...
def run_full_grid():
    ...
    # Write raw CSV header once, then append after every single run.
    # This means a Colab disconnect only loses the run in progress,
    # not everything completed so far.
    fieldnames = ["model", "shots", "degradation", "seed",
                  "accuracy", "f1", "train_time_sec", "num_params"]
    write_header = not os.path.exists(RAW_CSV)
    raw_file = open(RAW_CSV, "a", newline="")
    raw_writer = csv.DictWriter(raw_file, fieldnames=fieldnames)
    if write_header:
    ...
    if os.path.exists(RAW_CSV):
        with open(RAW_CSV, "r") as f:
            ...
            for row in reader:
                key = (row["model"], row["shots"], row["degradation"], row["seed"])
    ...
    for model_name in MODELS:
        ...
        train_base, test_base = random_split(
            ...
            generator=torch.Generator().manual_seed(0)  # fixed, not swept
        )

        normalize_after = ([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]) if cfg["normalize"] else None

        for n_shot in LABEL_COUNTS:
            for degradation in DEGRADATION_LEVELS:
                for seed in SEEDS:
                    ...
                    print(f"Running: model={model_name}, shots={shots_label}, "
                          f"degradation={degradation}, seed={seed}")
                    ...
                    metrics = train_and_eval(model, train_loader, test_loader, epochs=epochs)
    ...
    raw_file.close()
    print(f"\nAll runs complete. Raw results in {RAW_CSV}")
...
def summarize_results():
    ...
    with open(RAW_CSV, "r") as f:
        reader = csv.DictReader(f)
        for row in reader:
            key = (row["model"], row["shots"], row["degradation"])
            grouped[key]["acc"].append(float(row["accuracy"]))
            grouped[key]["f1"].append(float(row["f1"]))

    with open(SUMMARY_CSV, "w", newline="") as f:
        fieldnames = ["model", "shots", "degradation", "n_seeds",
                      "accuracy_mean", "accuracy_std", "f1_mean", "f1_std"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for (model, shots, degradation), vals in sorted(grouped.items()):
            writer.writerow({
                "model": model,
                "shots": shots,
                "degradation": degradation,
                "n_seeds": len(vals["acc"]),
                "accuracy_mean": round(np.mean(vals["acc"]), 4),
                "accuracy_std": round(np.std(vals["acc"]), 4),
                "f1_mean": round(np.mean(vals["f1"]), 4),
                "f1_std": round(np.std(vals["f1"]), 4),
            })

    print(f"Summary (mean +/- std across seeds) saved to {SUMMARY_CSV}")
