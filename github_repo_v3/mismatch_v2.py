...
def make_degraded_dataset(base_dataset, level, normalize_after=None):
    """level can be 'none'/'moderate'/'severe' (fixed) or 'mixed' (randomly
    picks none/moderate/severe PER IMAGE, PER EPOCH - simulates training on
    a realistic mix of conditions as a mismatch-mitigation strategy)."""
    class Wrapped(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            if self.level == "mixed":
                chosen = random.choice(["none", "moderate", "severe"])
                img = DegradeTransform(chosen)(img)
            else:
                img = DegradeTransform(self.level)(img)
...
V2_SHOTS = [5, None]  # the two extremes anchoring the mismatch claim; NOT
                       ...
                       # limitations section for that explicitly-scoped gap
V2_TRAIN_DEGRADATION = ["none", "moderate", "severe", "mixed"]
V2_TEST_DEGRADATION = ["none", "moderate", "severe"]
V2_SEEDS = [42, 123, 7, 2024, 99, 17, 256, 512, 88, 314]  # 10 seeds, matches main grid

V2_CSV = "/content/drive/MyDrive/results_mismatch_v2.csv"
...
def run_mismatch_v2():
    ...
    fieldnames = ["model", "shots", "train_degradation", "test_degradation", "seed", "lr",
                  "accuracy", "f1", "train_time_sec", "num_params"]
    f, writer, already_done = open_resumable_csv(V2_CSV, fieldnames)
    ...
    for model_name in V2_MODELS:
        ...
        for n_shot in V2_SHOTS:
            for train_deg in V2_TRAIN_DEGRADATION:
                for seed in V2_SEEDS:
                    shots_label = n_shot if n_shot else "full"

                    needed_test_degs = [
                        td for td in V2_TEST_DEGRADATION
    ...
    f.close()
    print(f"\nMismatch v2 complete. Results in {V2_CSV}")
